...
export default function ObjectDataBinding(){
    ...
    return (
        <>
            ...
            <div className="title">
                <label>이름 : </label>
                <input id="name" name="name" placeholder="이름" onChange={handleChange}/>
                <br />
                {isNameValid ? null : <p>이름은 2글자 이상 적어주세요</p>}

                <label>나이 : </label>
                <input id="age" type="number" name="age" placeholder="나이" onChange={handleChange} />
...
function SearchForm(){
    /* 
        useRef
         ...
         - 입력값을 실시간으로 반영하지 않고, 특정 시점에만 가져올 수 있기 때문에 불필요한 렌더링을 방지할 수 있다.
   ...
   return (
        <form onSubmit={handleSearch}>
            <input ref={keywordRef} type="text" placeholder="검색어 입력"/>
            <button>검색</button>
