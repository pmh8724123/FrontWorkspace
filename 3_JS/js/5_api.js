...
test4.onclick = function(){
    ...
    console.log(7 % 'a'); // 숫자형태가 아닌 문자열을 숫자로 형변환 시 
    // 반환되는 값 (NaN). NaN(number)과 기타 값들간의 연산결과는 항상 NaN이 반환.
    console.log(Number('a')); // NaN
    

    console.log(7 + true); // ture는 Number로 변환 시 1로 처리
...
test5.onclick = function(){
    ...
    // 단, 변환불가 문자가 있는 경우 NaN을 반환

    console.log(parseInt(num)); // 1234
    // 문자열을 정수로 변환. 
    // 단, 변환불가 문자가 있는 경우 해당문자의 좌측까지만 변환.
...
test8.onclick = function(){
    /* 
        ...
        엄격 동등비교 연산자
         ===, !==
         왼쪽 오른쪽 타입이 서로 다르다면, 바로 fasle를 반환하는 연산자(===기준)
