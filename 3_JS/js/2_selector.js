...
document.querySelector("#id").onclick = function () {
    ...
    // 2. querySeletor('css선택자') css선택자와 일치하는 요소 1개 선택
    ...
    // 선택한 요소의 스타일 속성 변경
    li.style.color = '#fff';
    li.style.background = 'magenta';

    // 3. id 속성값으로 접근하기.
    // - 브라우저는(몇몇 브라우저 제외) id값의 요소를 전역변수형태로 등록
    //   하기 때문에 id속성값을 통해 요소를 바로 획득할 수 있다.
    // - 단, 모든브라우제서 지원하지 않고, 또 다른 변수와 이름이 충돌될 수 있기 때문에
    //   주의해야한다.
...
document.querySelector("#email-confrim").onclick = function () {
    //  *  - 아이디로 접근
    var userEmail = document.querySelector("#userEamil");
