...
type MBTIType1 = {mbti:string, feature:string[], vocation?:string};
const type1:MBTIType1  = {mbti : 'ISTP', feature : ['망상가', '공감능력 부족'], vocation : '데이터분석가'};
const type2:MBTIType1  = {mbti : 'INTJ', feature : ['완벽주의자', '무신경']};

type MBTIType2 = {mbti:string, bestGunghab:string[], worstGunghab?:string};
...
// 각 인터페이스에 들어가는 속성들은 mkm,karina,winter객체에 대입되는 객체를 바탕으로 유추해보세요.
...
function greetUser({name, age, ...others} : {
    name : string, age : number, [key:string] : string | number
}) {
    console.log(`안녕하세요. 제이름은 ${name} ${age}살 이빈다.`); // 안녕하세요. 제이름은 홍길동 30살 입니다.
