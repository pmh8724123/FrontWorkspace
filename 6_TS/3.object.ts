...
const person1:Person = {name:'pmh', age:18};
const person2:Person = {name:'pmh', age:18, gender:'male'};
...
interface Bird{
    name : string;
    // 선언병합 시 기존 인터페이스가 가지고 있는 속성의 타입을 반드시 일치시켜줘야한다.
    // wing : boolean; 
}
...
// 속성에 여러값들이 들어가는 경우, 인덱스 시그니쳐는 그 모든값들에 대해 표현할 수 있어야 한다.
type ClassInfo = {
    name : string,
    classRoom : string,
    teacher : string,
