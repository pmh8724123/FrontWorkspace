...
const tiger = new Tiger('pmh', 25, '경기도');
tiger.bark();

/* 
    2. 생성자 파라미터 문법
     - class생성 시 반복되는 코드를 줄여주는 단축구문
*/
class Product {
    // 필드선언 생략
    // 초기화 코드 생략
...
const result = returnGeneric<string>("hello"); // 자동 타입추론이 이루어진다 <string> 생략가능
...
function config<T extends Options = {theme:'dark'}>(options?:T):T{
    ...
    // 널병합연산자 ??
    //  - a ?? b -> a가 null 또는 undefined가 아니라면 A, 맞다면 B반환
    return  options ?? {theme:"dark"} as T;
};
...
/* 
    4. Utility Type
     - 기존 객체타입들을 변형 및 조작할 수 있도록 도와주는 유틸리티 타입 
*/
     
/* 
    1) Partial<T>
      - T의 모든 속성을 optional속성으로 변경해주는 유틸리티 타입
      - 모든 속성을 필수, 읽기전용으로 만들어 주는 Required<T>, Readonly<T>도 존재한다.
*/
interface User{
    id : number;
    name : string;
...
/* 
   3) Omit<T,K>
    - 객체타입에서 특정 속성을 제외한 나머지만 남긴 새 타입을 만들 때 사용 
*/
type NewUser = Omit<User, "id">;
const userData:NewUser = {name:"pmh", email:"zz"};
...
*/
function getUser(){
    return {id:1, name:'pmh', email:"aaa"};
}
