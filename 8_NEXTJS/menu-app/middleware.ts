...
/* 
    #1. Middleware
     - 서버컴포넌트가 렌더링 되거나, API가 실행되기 전에 요청을 가로채서 특정 로직을 수행할 수 있는 서버측 도구(필터역할수행)
     - 프론트 서버의 인증 및 권한관리, 테스트 등을 수행한다.
     - NextRequest : 가로챈 요청내용
     - NextResponse : 로직 수행 후 다음 단계로 넘길 때 사용하는 객체
*/
export async function middleware(request:NextRequest){
    ...
    /* 
        1. 액세스 토큰은 만료되어 없으나, 리프레쉬 토큰만 남아 있는 경우
         - 그대로 api요청을 하면 401에러가 발생하므로, 미들웨어에서 쿠키 재발급요청을 보낼 예정이다.
    */
   if(!accessToken && refreshToken){
        ...
        if(refreshRes.ok){
            ...
            if(setCookie){
                ...
                // 리프레쉬 실패 시 쿠키 삭제 후 로그인 페이지 리디렉트
