...
import { useUserInforActions } from '../hooks/useUserInfoActions';
...
export default function Login(){
    ...
    const handleLoginSync = useUserInforActions();
        ...
        .then( response => {
            ...
            /* 
                #1. JWT
                 ...
                 - JWT를 활용한 인증/인가 매커니즘은 세션기반 로그인방식과 달리 서버가 인증상태를 따로 저장할 필요가 없기 때문에 무상태서버 구현이 가능하다.
                 - 발급된 JWT토큰은 매 요청시 함께 전달되어 인증에 사용한다.

                #2. JWT저장 위치
                 - JWT토큰은 클라이언트의 브라우저에서 관리해야하는 데이터이며, API요청에 사용되는 중요한 데이터이다.
                    ...
                    2) sessionStorage
                    - 탭단위 세션 저장소로 탭을 닫으면 데이도 삭제된다. (데이터유지시간이 짧다)
                    - xss공격에 취약하다. 따라서 적절한 저장소가 아니다 
                    
                    3) 쿠키
                    - 쿠키는 특정 도메인의 경로에 지정된 시간동안만 저장되는 데이터
                    - HTTP통신 시 항상 자동으로 전달
                    - 기본쿠키는 JS로 접근하여 탈취될 수 있으나, http-only설정 시 js로 쿠키제어 불가능(xss에서 안전)

                    4) 리액트메모리
                    - 리덕스에 보관 시 메모리에만 저장되며, 새로고침 시 데이터가 소멸한다.
                    - xss공격의 위험이 존재하긴 하나 유지시간이 굉장히 짧기 때문에, 공격을 최소화 할 수 있다.

                #3. AccessToken과 RefreshToken
                 - JWT기반 인증에서 사용하는 토큰들
                 - AccessToken실제 인증에 사용하는 토큰으로, 유지시간을 짧게 설정하여 탈취 시 부담을 줄여야한다.
                 - RefreshToken은 accessToken이 만료된 경우 이를 갱신하기 위한 토큰
