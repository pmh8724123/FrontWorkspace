...
/* 
    # Client Component
     - 모든 컴포넌트의 기본값은 서버컴포넌트이며, 클라이언트 컴포넌트로 만들려면 파일 최상단에 'use client'를 작성해야 한다.
            ...
            <div id="header-container">
                ...
                <div className="navbar bg-light navbar-expand">
                    <ul className="navbar-nav">
                        ...
                        {   
                            /* 
                                #1. 조건부 렌더링 사용 시 하이드레이션 에러 발생
                                 - 서버컴포넌트의 상태와 하이드레이션 시 결과가 다른 경우 발생하는 에러 -> isAutenticated는 redux에 존재하는 데이터인데,
                                   이 코드는 클라이언트 컴포넌트에서만 알 수 있기 때문
                                 - 하이드레이션에러블 방지하기 위해선, 서버, 클라이언트 모두 동일한 값을 보게 해야한다. => mounted설정
                            */
                                ...
                                <li className="nav-item" >
                                    {/* 
                                        #2. Image
                                         ...
                                         - 이미지가 실제 화면에 나타날 때까지 로딩을 지연하는 지연로딩기능
                                         - 사용자의 기기에 맞춰 적절한 크기의 이미지를 자동생성
                                         - 이미지가 로드되면서 화면이 덜컥거리는 현상을 방지해주고, 브라우저에서 지원하는 경우 고품질 포멧으로도 변환
                                         - 최적화된 이미지는 .next/cache/images에 저장된다.
                                           이 때 변경한 이미지의 파일명이 그대로인 경우 캐싱된 이미지가 서비스되어 변경사항이 반영되지 않을 수 있다
                                        해결방법 ) 이미지 변경 시 변경사항을 함께 저장하여 이미지 url뒤에 붙여준다.
                                        이럴 경우, next.js와 브라우저는 새로운 이미지로 인식하게 된다.
